...
PASSWD_DESC=			Enable auth via /etc/passwd
MYSQL_DESC=			Enable auth via MySQL
PGSQL_DESC=			Enable auth via PostgreSQL
SYBASE_DESC=			Enable auth via Sybase
...
MYSQL_REPLICATION_DESC=		Enable MySQL database replication
MYSQL_LIMITS_DESC=		Enable MySQL mailbox limitations
ORACLE_DESC=			Enable auth via Oracle
LDAP_DESC=			Enable auth via LDAP
LDAP_SASL_DESC=			Enable auth via LDAP SASL
VALIAS_DESC=			Enable valias processing
ROAMING_DESC=			Enable roaming users support
IP_ALIAS_DESC=			Enable IP alias support
QMAIL_EXT_DESC=			Enable qmail-like user-* addresses
FILE_LOCKING_DESC=		Enable file locking
FILE_SYNC_DESC=			Enable fsync (decreases performance)
USERS_BIG_DIR_DESC=		Enable hashing of user directories
SEEKABLE_DESC=			Make input to vdelivermail seekable
SPAMASSASSIN_DESC=		Enable SpamAssassin support
SUID_VCHKPW_DESC=		Set vchkpw setugid vpopmail:vchkpw
SMTP_AUTH_PATCH_DESC=		Swap Challenge/Response for CRAM-MD5
ONCHANGE_SCRIPT_DESC=		Enable vpopmail/etc/onchange scripts
FPIC_DESC=			Compile with -fPIC
MAILDROP_DESC=			Enable with maildrop MDA support
DOMAIN_QUOTAS_DESC=		Enable non-system domain quotas
